...
if uploaded_file:
    try:
        ...
        if header_row_index is not None:
            ...
            # --- الخوارزمية المحدثة: تقسيم العطل لضمان الاستمرارية (Fragmentation) ---
            processed_data = []
            if all_events:
                    ...
                    for _, ev in group.iterrows():
                        ...
                        if not current_timeline:
                            ...
                            if ev_dict['Start'] < last['End']:
                                if ev_dict['Pri'] < last['Pri']: # الجديد أهم (مثل Declenchement)
                                    old_end = last['End']
                                    last['End'] = ev_dict['Start'] # قص العطل القديم عند بداية الجديد
                                    
                                    current_timeline.append(ev_dict) # إضافة العطل الجديد المهم
                                    
                                    # الجزء المهم: إذا كان العطل القديم ينتهي بعد الجديد، نعيد العطل القديم
            ...
            if processed_data:
                ...
                st.success("✅ Analyse terminée : Fragmentation active (X -> طارئ -> X)")
